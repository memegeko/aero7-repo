...
| Computer Management | `fd5da379f73e326252bfa6de943bce6cd555e6be` | `aero7-computer-management-git` | Separate MIT administration console with real systemd, journald, Samba, procfs, Linux account, UDisks2, polkit, and Device Manager integrations; all six tests pass and every selectable module installs a searchable application shortcut with its matching theme icon |
...
- The original companion audit's repository validation, lock checks,
  ...
  passed for its 20-package set. The later Aero7 Desktop/Gadgets and Internet
  Explorer identity update raises the source manifest to 22 packages and has
...
Run the normal builder workflow, sign the 22-package repository, and install
